...
| 质量校验 | 8 条规则，命中 9 条问题记录，全部逐条留痕 |
| 展示页面 | `site/index.html`，纯静态、可离线打开 |
| 网页测试 | Playwright 16 个用例 × 2 种视口，共 32 条全部通过 |
...
3. **清洗**：`--`、`暂无` 等 16 种占位符统一为缺失值；日期归一为 ISO 格式；数值去掉千分位与单位；
   ...
   与三个布尔标记。
4. **校验**：八条规则逐条检查（见下），命中项写入 `data/issues.csv`，不做静默丢弃。
...
| R8 | 公告日期合理性（不早于 1990-01-01） | 命中 1 条：平安银行 1991 年送股记录的公告日期被源站写作 `1900-01-01` |
...
* 源站对未知公告日期使用 `1900-01-01` 占位（本次命中 1 条）。该记录保留在数据集中并标记
  「公告日期存疑」，但不参与分红年度派生与日期区间统计。
